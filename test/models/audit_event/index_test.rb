...
class AuditEvent::IndexTest < ActiveSupport::TestCase
  ...
  test "events returns audit events for the company" do
    index = AuditEvent::Index.new(@company)
    assert index.events.any?
    index.events.each do |event|
      assert_equal @company.id, event.company_id
    end
  end
  ...
  test "filters by actor_type" do
    index = AuditEvent::Index.new(@company, actor_type: "User")
    index.events.each do |event|
  ...
  test "filters by action" do
    index = AuditEvent::Index.new(@company, action_filter: "created")
    index.events.each do |event|
  ...
  test "filters by date range" do
    ...
    index = AuditEvent::Index.new(@company, start_date: start_date, end_date: end_date)
    index.events.each do |event|
  ...
  test "returns all events with no filters" do
    index = AuditEvent::Index.new(@company)
    all_events = AuditEvent.for_company(@company)
    assert_equal all_events.count, index.events.size
  end

  test "ignores date range when only start_date provided" do
    index = AuditEvent::Index.new(@company, start_date: Date.current.to_s)
    all_count = AuditEvent.for_company(@company).count
    assert_equal all_count, index.events.size
  end

  test "ignores date range when only end_date provided" do
    index = AuditEvent::Index.new(@company, end_date: Date.current.to_s)
    all_count = AuditEvent.for_company(@company).count
    assert_equal all_count, index.events.size
  end
  ...
  test "filtered? returns true with date range" do
    index = AuditEvent::Index.new(@company, start_date: "2025-01-01", end_date: "2025-12-31")
    assert index.filtered?
  end
  ...
  test "any_events? returns false when no events match" do
    index = AuditEvent::Index.new(@company, action_filter: "nonexistent_action_xyz")
  ...
  test "start_date returns the filter value" do
    index = AuditEvent::Index.new(@company, start_date: "2025-01-01")
    assert_equal "2025-01-01", index.start_date
  end

  test "end_date returns the filter value" do
    index = AuditEvent::Index.new(@company, end_date: "2025-12-31")
